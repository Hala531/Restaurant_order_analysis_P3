...
**Situation** : i've just been hired as a data Analyst for 'the taste of the world café' . a restaurant that has  divers menu offerings and serves generous potions.  
**Assignment** : the taste of the world café debuted a new menu at the start of the year, i've been asked to dig into the customer data to see whivh menu items are doing well / not well and what the customers seem to like best.  
**Objectives** : 
...
-- 1: write a querry to find the total number of items on the menu 

SELECT 
COUNT(*) AS total_items -- coun(*) compte le nombre total de lignes 
FROM menu_items;
-- 2 : write a querry to find the least expensive item on the menu
SELECT item_name, price
FROM menu_items
ORDER BY price ASC 
LIMIT 1;
--3 : write a querry to find the most expensive item on the menu 
SELECT item_name, price 
FROM menu_items 
ORDER BY price DESC 
LIMIT 1; 
--4 : write a querry to find how many italien dishes are on the manu
SELECT category, 
COUNT(category)
FROM menu_items
GROUP BY 1 
...
--5 write a querry to find the least expensive italian dish on the menu 
SELECT 
item_name,
price
FROM 
...
ORDER BY price ASC 
LIMIT 1
--6 write a querry to find the most expensive italian dish on the menu 
SELECT 
item_name, 
price 
FROM 
...
menu_items
GROUP BY 1;
--8 what is the average dish price in each category 
SELECT 
category, 
...
--1 write a querry to find the date range of the table order_details 
SELECT 
...
--3 How many unique items were ordered within this date range
...
--2 What were the least ordered items (we should not keep the chicken tacos on our menu)
...
LIMIT 2
--3 what were the most ordered items (we should keep the hamburger on our menu)
SELECT  
m.item_name,
m.category,
COUNT( o.order_details_id) AS most_ordered_items
